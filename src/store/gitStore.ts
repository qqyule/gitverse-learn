...
import type { GitStore, GitHash, GitCommit, FileStructure, Branch, FileEntry } from '@/types/git';
  ...
  immer((set, get) => ({
    ...
    commit: (message) => {
      ...
      set((state) => {
        ...
        const parentHash = currentBranch?.headCommitHash || (state.HEAD.type === 'detached' ? state.HEAD.ref : null);
        const hash = generateHash();
    ...
    branch: (name) => {
      ...
      set((state) => {
        const currentCommitHash = state.HEAD.type === 'branch'
    ...
    merge: (sourceBranch) => {
      ...
      set((state) => {
        ...
        while (current) {
          ...
          current = current.parents[0] ? state.commits[current.parents[0]] : undefined as any;
        }
    ...
    reset: (mode, ref) => {
      set((state) => {
        const targetRef = ref || (state.HEAD.type === 'branch' 
          ? state.branches[state.HEAD.ref].headCommitHash 
          : state.HEAD.ref);
    ...
    log: () => {
      ...
      let currentHash = state.HEAD.type === 'branch'
        ? state.branches[state.HEAD.ref]?.headCommitHash
        : state.HEAD.ref;
    ...
    executeCommand: (command) => {
      ...
      if (cmd !== 'git') {
        set((s) => { s.lastOutput = `Command not found: ${cmd}`; });
      ...
      switch (gitCmd) {
        ...
        case 'commit':
          if (gitArgs[0] === '-m' && gitArgs[1]) {
            const message = gitArgs.slice(1).join(' ').replace(/^[\"']|[\"']$/g, '');
            get().commit(message);
          } else {
            set((s) => { s.lastOutput = 'Usage: git commit -m \\"message\\"'; });
          }
          break;
        case 'checkout':
        ...
        case 'status':
          ...
          break;
        default:
          set((s) => { s.lastOutput = `git: '${gitCmd}' is not a git command`; });
      }
    ...
    getHeadCommit: () => {
      const state = get();
      const hash = state.HEAD.type === 'branch'
        ? state.branches[state.HEAD.ref]?.headCommitHash
        : state.HEAD.ref;
