...
export function FileDeck({ className, onFileClick }: FileDeckProps) {
  const { workingDirectory, stagingArea, add } = useGitStore();
  ...
  if (files.length === 0) {
    return (
      <div className={cn("flex items-center justify-center text-muted-foreground", className)}>
        <p className="text-sm">No files in working directory</p>
      </div>
    );
  }
  ...
  return (
    <div className={cn("space-y-2", className)}>
      ...
      <div className="grid gap-2">
        {files.map(([path, file], index) => (
          <motion.div
            ...
            className={cn(
              "file-card flex items-center gap-3 p-3 rounded-lg border cursor-pointer",
      ...
      {Object.keys(stagingArea).length > 0 && (
        <div className="mt-6">
          <h3 className="text-sm font-medium text-muted-foreground mb-2">Staging Area</h3>
          <div className="grid gap-2">
